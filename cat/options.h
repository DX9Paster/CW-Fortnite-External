...
namespace options
{
	...
	bool ignoredownplayers = false;
	bool RandoAim = false;
	bool controllerpsxb = false;
	int conrollertig;
	int AimFOV = 150;
	bool drawfov = true;
	int mousesmoothness = 7.0f;
	float Shape = 50.0f;
	
	float outline5[3] = { color.Black[0], color.Black[1], color.Black[2] };
	float Visible[3] = { color.Red[0], color.Red[1], color.Red[2] };
	float NonVisible[3] = { color.white[0], color.white[1], color.white[2] };
	...
	float width = 0.500;
	int MaxEspDistance = 175;
	float ThicknessSkeleton = 2;
	float ThicknessHeaddot = 1.0f;
	float Thicknessbox = 1.5;
	float Thicknessesp = 1.0f;
}